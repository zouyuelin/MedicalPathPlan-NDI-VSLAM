...
bool MouseInteractorStyleCenterline::finished = false;
...
//    this->Interactor->CreateRepeatingTimer(30);
    ...
    cameraSensorBefore = vtkSmartPointer<vtkSphereSource>::New();
    cameraSensorBefore->SetRadius(1.5);

    vtkSmartPointer<vtkPolyDataMapper> cameraSensorMapperBefore = vtkSmartPointer<vtkPolyDataMapper>::New();
    cameraSensorMapperBefore->SetInputConnection(cameraSensorBefore->GetOutputPort());

    cameraSensorActorBefore = vtkSmartPointer<vtkActor>::New();
    cameraSensorActorBefore->SetMapper(cameraSensorMapperBefore);
    cameraSensorActorBefore->GetProperty()->SetColor(1, 0, 0);
    cameraSensorActorBefore->SetVisibility(0);
...
void MouseInteractorStyleCenterline::OnLeftButtonDown()
{
    ...
    {
        this->GetCurrentRenderer()->AddActor(cameraSensorActor);
        this->GetCurrentRenderer()->AddActor(cameraSensorActorBefore);
        ready = true;
        onceInit = false;
    }
}
...
void MouseInteractorStyleCenterline::OnKeyPress()
{
    ...
    {
        SetResetStatus(true); //reset the posture
    }
    // roaming
    else if (key == "x")
    {
    ...
    else if (key == "c")
    {
        ...
        if(camType == CAMERA_TYPE::IN_HAND)
        {
            ...
            cameraSensorActorBefore->SetVisibility(1);
            this->GetCurrentRenderer()->ResetCamera();
            this->GetInteractor()->Render();
    ...
    {
        std::ofstream out("data/" + std::to_string(fileName) + "_source.txt");
        vtkSmartPointer<vtkPoints> recordPoints = vtkSmartPointer<vtkPoints>::New();
        recordPoints->DeepCopy(mPoints[pathSelect]);
        for (int i = 0; i < recordPoints->GetNumberOfPoints(); ++i) {
            double* point = new double[3];
            recordPoints->GetPoint(i,point);
            out<<i<<" "<< point[0] <<" "<<point[1]<<" "<<point[2]<<std::endl;
        }
        out.close();

        std::ofstream all("data/AllSource.txt");
        for(int j = 0; j<mPoints.size();j++)
        {
            vtkSmartPointer<vtkPoints> recordPoints = vtkSmartPointer<vtkPoints>::New();
            recordPoints->DeepCopy(mPoints[j]);
            for (int i = 0; i < recordPoints->GetNumberOfPoints(); ++i) {
                double* point = new double[3];
                recordPoints->GetPoint(i,point);
                all<<i<<" "<< point[0] <<" "<<point[1]<<" "<<point[2]<<std::endl;
            }
        }
        all.close();
...
void MouseInteractorStyleCenterline::castCamera(Eigen::Vector3d position,Eigen::Vector3d focalPoint,Eigen::Vector3d up)
{
    ...
    double wcx = -2*1.17 / 600;
    double wcy = -2*6.12/ 600;
    double viewAngle = 2.0 * std::atan2(300,397) * 180.f / M_PI;
...
vtkPolyData* MouseInteractorStyleCenterline::GetSurface()
{
    return m_surface;
}
