...
//        cout<<"Fucking inside ! "<<++i << endl;
    ...
    else
    {
        ...
        if(m_lastPosition_change.norm() == 0 || lastPositionInside == true) //m_lastPosition_change.norm() == 0 || lastPositionInside == true
        {
        ...
        else if(lastPositionInside == false)
        {
            ...
            {
                Eigen::Vector3d theBorderPt;
                Eigen::Vector3d theBorderPtNor;

                Eigen::Vector3d theBorderPt_p;
                Eigen::Vector3d theBorderPt_pNor;
                ...
                # pragma omp parallel sections
                      ...
                      //*****线程1：******//
                           #pragma omp section
                           {
                               theBorderPt = getTheClosetPoint(newDirection[0],newDirection[1],newDirection[2]);
                               theBorderPtNor = getTheClosetPointNorm(newDirection[0],newDirection[1],newDirection[2]);
                           }

                       //*****线程2：******//
                           #pragma omp section
                           {
                               theBorderPt_p = getTheClosetPoint(position[0],position[1],position[2]);
                               theBorderPt_pNor = getTheClosetPointNorm(position[0],position[1],position[2]);
                           }
                       }


                Eigen::Vector3d newVector = theBorderPt - m_lastPosition_change;

                double theta = positionDirection.transpose() * newVector;//
                theta = theta / (positionDirection.norm()*newVector.norm());

                if(theta > std::cos(1/16.F*M_PI))
                {
                    m_lastPosition_change = theBorderPt_p - 1.5*theBorderPt_pNor;
                }
                else
                {
                    m_lastPosition_change = theBorderPt - 1.5*theBorderPtNor;
                }
