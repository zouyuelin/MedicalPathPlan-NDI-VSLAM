...
void DLrun::objectPositionONNX(cv::Mat &imgSource)
{
    //输入网络的维度
    static constexpr const int width = 600;
    static constexpr const int height = 600;
    static constexpr const int channel = 3;
    static std::array<int64_t, 4> input_shape_{ 1,height, width,channel};
        ...
        for (int i = 0; i < num_detection[0]; i++)
            {
                ...
                if (confidence >= 0.05)
                {
                    ...
                    cv::String label = cv::String("confidence :") +std::to_string(confidence);
                    int baseLine = 0;
